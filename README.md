...
```mermaid
graph TD
    User((Người dùng)) --> Perception[Perception Layer]
    Perception --> |Chuẩn hóa Goal| Brain[Planning & Reasoning]
    Brain --> |Chọn Công cụ| Action[Execution Layer]
    Action --> |Thực thi Tool| DB[(PostgreSQL)]
    DB --> |Kết quả| Action
    Action --> |Observations| Evaluator[Evaluation Layer]
    Evaluator --> |Dữ liệu không đủ| Brain
    Evaluator --> |Đạt mục tiêu| Final[Kết quả Cuối cùng]
    Final --> |Lưu bài học| Learning[Learning Layer]
    Learning --> |Kinh nghiệm| Brain
    Final --> User
```
...
## 🏗️ Chi tiết các Tầng (Operational Layers)

1. **Perception**: Chuẩn hóa NLP và phát hiện Role.
2. **Planning**: Sử dụng Semantic Router và RAG để chọn logic.
3. **Execution**: Dynamic Tool Registry (Search, Stats, Orders).
4. **Learning**: Vector Similarity Memory (Cosine Similarity).
...
$env:APP_PORT="8000"
$env:DATABASE_URL="postgresql://postgres:123456@localhost:5432/agent_db"
